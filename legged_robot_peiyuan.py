...
class LeggedRobotBase(BaseTask):
    def __init__(self, config, device):
        ...
        if self.debug_curve:
            ...
            self.left_knee_joint = 1
            self.left_ankle_pitch_joint = 2
            self.right_hip_pitch_joint = 3
            self.right_knee_joint = 4
            self.right_ankle_pitch_joint = 5
            # 初始化绘图
            plt.ion()  # 打开交互模式
    ...
    def _init_buffers(self):
        ...
        self.actions_absolute = torch.zeros(self.num_envs, self.dim_actions, dtype=torch.float, device=self.device, requires_grad=False)
        ...
        self.last_actions_absolute = torch.zeros(self.num_envs, self.dim_actions, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _init_domain_rand_buffers(self):
        ...
        self._rfi_torques = torch.zeros(self.num_envs, self.num_dof, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _physics_step(self):
        ...
        if self.config.domain_rand.randomize_ctrl_delay_step_decimation:
            action_delay_decimation = torch.randint(self.config.domain_rand.ctrl_delay_step_decimation[0],
                                                    self.config.domain_rand.ctrl_delay_step_decimation[1]+1, 
                                                    (self.num_envs,), device=self.device, requires_grad=False)
        for idx in range(self.config.simulator.config.sim.control_decimation):
            self.actions_after_delay[idx == action_delay_decimation] = self.actions[idx == action_delay_decimation].clone()
            self.ref_joint_pos_cur[idx == action_delay_decimation] = self.ref_joint_pos[:,0][idx == action_delay_decimation].clone()
    ...
    def _draw_curve(self):
        if self.debug_curve:
            ...
            self.left_hip_action.append(self.actions[0, self.left_hip_pitch_joint].item() * 0.25 + self.default_dof_pos[0, self.left_knee_joint].item())
            self.left_hip_state.append(self.simulator.dof_pos[0, self.left_hip_pitch_joint].item())
            self.left_hip_ref.append(self.ref_joint_pos[0, 0, self.left_hip_pitch_joint].item())
            self.left_knee_action.append(self.actions[0, self.left_knee_joint].item() * 0.25 + self.default_dof_pos[0, self.left_knee_joint].item())
            self.left_knee_state.append(self.simulator.dof_pos[0, self.left_knee_joint].item())
            self.left_knee_ref.append(self.ref_joint_pos[0, 0, self.left_knee_joint].item())
            self.left_ankle_action.append(self.actions[0, self.left_ankle_pitch_joint].item() * 0.25 + self.default_dof_pos[0, self.left_ankle_pitch_joint].item())
            self.left_ankle_state.append(self.simulator.dof_pos[0, self.left_ankle_pitch_joint].item())
            self.left_ankle_ref.append(self.ref_joint_pos[0, 0, self.left_ankle_pitch_joint].item())

            # stand_mask = self._get_gait_phase()
            self.right_hip_action.append(self.actions[0, self.right_hip_pitch_joint].item() * 0.25 + self.default_dof_pos[0, self.right_knee_joint].item())
            self.right_hip_state.append(self.simulator.dof_pos[0, self.right_hip_pitch_joint].item())
            self.right_hip_ref.append(self.ref_joint_pos[0, 0, self.right_hip_pitch_joint].item())
            self.right_knee_action.append(self.actions[0, self.right_knee_joint].item() * 0.25 + self.default_dof_pos[0, self.right_knee_joint].item())
            self.right_knee_state.append(self.simulator.dof_pos[0, self.right_knee_joint].item())
            self.right_knee_ref.append(self.ref_joint_pos[0, 0, self.right_knee_joint].item())
            self.right_ankle_action.append(self.actions[0, self.right_ankle_pitch_joint].item() * 0.25 + self.default_dof_pos[0, self.right_ankle_pitch_joint].item())
            self.right_ankle_state.append(self.simulator.dof_pos[0, self.right_ankle_pitch_joint].item())
            self.right_ankle_ref.append(self.ref_joint_pos[0, 0, self.right_ankle_pitch_joint].item())
    ...
    def _post_compute_observations_callback(self):
        ...
        self.last_actions_absolute[:] = self.actions_absolute[:]
    ...
    def _reset_buffers_callback(self, env_ids, target_buf=None):
        if target_buf is not None:
            ...
            self.actions_absolute[env_ids] = target_buf["actions_absolute"].to(self.actions_absolute.dtype)
            self.last_actions_absolute[env_ids] = target_buf["last_actions_absolute"].to(self.last_actions_absolute.dtype)
            ...
            self.actions_absolute[env_ids] = 0.
            self.last_actions_absolute[env_ids] = 0.
    ...
    def get_mppi_buffers(self, env_ids):
        ...
        return {
            ...
            "actions_absolute": copy.deepcopy(self.actions_absolute[env_ids]),
            "last_actions_absolute": copy.deepcopy(self.last_actions_absolute[env_ids]),
    ...
    def _compute_observations(self):
        ...
        for obs_key, obs_config in self.config.obs.obs_dict.items():
            ...
            if obs_key == "critic_obs" and not self.config.obs.dagger_critic_noise:
                noise_extra_scale = 0.
        ...
        if self.config.obs.dagger_critic_noise:
            self.obs_buf_dict_raw['critic_obs'].update(self.obs_buf_dict_raw['actor_obs'])
        ...
        self.hist_obs_dict = self.obs_buf_dict_raw['actor_obs'].copy() #TODO fix the bug
    ...
    def _compute_torques(self, actions):
        ...
        if self.config.domain_rand.randomize_torque_rfi:
            self._rfi_torques[:] = (torch.rand_like(torques)*2.-1.) * self.config.domain_rand.rfi_lim * self._rfi_lim_scale * self.torque_limits
            torques = torques + self._rfi_torques.clone()
