...
from motion_lib.motion_lib_base import MotionLibBase, FixHeightMode
...
def quat_rotate_inverse(q, v):
    ...
    # term a
    a = v * (2.0 * q_w2 - 1.0).unsqueeze(1)   # [B, 3]
    
    # term b
    b = 2.0 * q_w.unsqueeze(1) * torch.cross(q_vec, v, dim=1)   # [B, 3]
    
    # term c: dot(q_vec, v) * q_vec
    dot_qv = torch.sum(q_vec * v, dim=1, keepdim=True)          # [B, 1]
    c = 2.0 * dot_qv * q_vec                                    # [B, 3]
    
    return a - b + c

def get_euler_xyz_fast(q: torch.Tensor) -> torch.Tensor:
    """
    q: [..., 4]，格式为 (qx, qy, qz, qw)
    返回：[..., 3]，(roll, pitch, yaw)
    """
    ...
    # pitch (y-axis rotation) —— clamp 替代 saturate+copysign
...
class MotionLibH1(MotionLibBase):
    ...
    def load_motions(self, skeleton_trees, gender_betas, limb_weights, random_sample=True, start_idx=0, max_len=-1, target_heading = None):
        ...
        self.length_ends = lengths.cumsum(0)
    ...
    def get_motion_state(self, motion_ids, motion_times, offset=None):
        ...
        # add future pose
        future_dist = 15
        fut_f0l = torch.min(f0l + future_dist, self.length_ends[motion_ids] - 1)

        ref_fut_dof_pos = self.dof_pos[fut_f0l]
        ref_fut_dof_vel = self.dvs[fut_f0l]
        ...
        root_rpy = get_euler_xyz_fast(rb_rot[..., 0, :].clone())
        local_root_vel = quat_rotate_inverse(rb_rot[..., 0, :].clone(), body_vel[..., 0, :].clone())
        local_root_ang_vel = quat_rotate_inverse(rb_rot[..., 0, :].clone(), body_ang_vel[..., 0, :].clone())
        
        return_dict.update({
            "root_rpy": root_rpy,
            "local_root_vel": local_root_vel,
            "local_root_ang_vel": local_root_ang_vel,
            ...
            "ref_fut_dof_pos": ref_fut_dof_pos,
            "ref_fut_dof_vel": ref_fut_dof_vel,
        })
        return return_dict
